...
$exp=Expect->spawn("/usr/bin/ssh dcuser\@eniqs ") or die "Can not SSH to $Machine  \n";
                if ($exp->expect($timeout, 'Are you sure you want to continue connecting (yes/no)?')) {
                                        ...
                                        $exp->expect($timeout,[ 'password: $' => sub {$exp->send("dcuser\n"); exp_continue; } ],
                                                '-re', qr'[^.*->#] $' => sub {
                                                                        exp_continue; }
                        );
     $exp->expect($undef, $PROMPT);
my $cmd_latest_epfg_rt_version = "/usr/bin/wget --user=statsjenki --password=April\@2019 -O - http://150.132.181.253/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/SOLARIS_baseline.html | grep epfg_rt | grep '.zip' ";
...
my $epfg_download = "/usr/bin/wget --user=statsjenki --password=April\@2019 -N http://150.132.181.253/view/www_eniq/vobs/ossrc/del-mgt/html/eniqdel/$Release/$fdm_version/$epfg_version -P /eniq/home/dcuser";
$exp->expect($undef, $PROMPT);
